...
const ShareProfileButton: React.FC<ShareProfileButtonProps> = ({ profileId, fullName }) => {
  ...
  return (
    <button
      onClick={handleShare}
      className="w-full flex items-center gap-3 px-4 py-2.5 md:px-4 md:py-2 bg-transparent md:bg-blue-50 text-slate-700 md:text-blue-600 hover:bg-slate-50 md:hover:bg-blue-100 rounded-xl font-semibold text-sm md:text-base transition text-left"
      title="Bagikan Profil"
    >
      {copied ? (
        <>
          <FiCheck className="text-green-600 animate-bounce" size={18} />
