...
const HelpCenter: React.FC = () => {
  ...
  // Data Mock FAQ (Disesuaikan dengan ekosistem KaryaMandiri)
  ...
  // Filter FAQ berdasarkan Pencarian dan Kategori Aktif
    ...
    <div className="min-h-screen pb-20">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="max-w-xl mx-auto relative mt-4">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 font-medium focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition text-sm"
            />
          </div>
        </div>
      </section>
      ...
      <main className="max-w-5xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <aside className="lg:col-span-1 space-y-3">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-2 mb-4">
            Kategori Bantuan
          </h3>
            ...
            {categories.map((cat) => (
              <button
                ...
                onClick={() => {
                  ...
                  setOpenFaqId(null); // Tutup semua accordion saat ganti kategori
                }}
              ...
              >
                <div className={`p-2 rounded-xl text-base ${activeCategory === cat.id ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                  {cat.icon}
                </div>
                <div>
                  <p className="block truncate">{cat.name}</p>
                  <span className={`text-[10px] font-medium block truncate mt-0.5 max-w-[160px] lg:max-w-none ${activeCategory === cat.id ? 'text-blue-100' : 'text-slate-400'}`}>
                    {cat.description}
                  </span>
                </div>
              </button>
        ...
        {/* MAIN CONTENT: FAQ ACCORDION LIST */}
        <section className="lg:col-span-2 space-y-4">
          ...
          {filteredFaqs.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 flex flex-col items-center justify-center space-y-3">
              ...
              <h4 className="font-bold text-slate-800 text-base">Hasil Tidak Ditemukan</h4>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full p-5 text-left flex justify-between items-center gap-4 font-bold text-slate-800 hover:text-blue-600 transition"
                    >
                      <span className="text-sm md:text-base leading-snug">{faq.question}</span>
                      <FiChevronDown 
                        className={`text-slate-400 transition-transform duration-300 shrink-0 text-lg ${isOpen ? 'rotate-180 text-blue-600' : ''}`} 
                      />
                    </button>
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="px-5 pb-5 pt-1 border-t border-slate-100 text-slate-600 text-sm md:text-base leading-relaxed whitespace-pre-line bg-slate-50/40">
                            {faq.answer}
                          </div>
                        </motion.div>
                      )}
          ...
          {/* EMERGENCY CONTACT / ESCALATION CARD */}
          <div className="bg-slate-900 text-white rounded-3xl p-6 md:p-8 mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
